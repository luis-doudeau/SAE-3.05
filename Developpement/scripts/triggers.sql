...
create trigger verifCreneauRepasStaff before insert on MANGER for each row 
  begin
    declare msg VARCHAR(300);
    if EXISTS(select * from REPAS, CRENEAU, STAFF
    where (new.idP = STAFF.idP and new.idRepas = REPAS.idRepas and REPAS.idCreneau = CRENEAU.idCreneau)
    and (TIME("00:00") > TIMEDIFF(TIME(dateDebut), TIME("11:30")) or TIMEDIFF(TIME(dateDebut), TIME("13:30")) > TIME("00:00") or
...
-- create trigger ajouteNavette after insert on ASSISTER for each row  
--   begin
